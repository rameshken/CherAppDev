...
module Users
  class RegistrationsController < Devise::RegistrationsController
    ...
    before_action :set_clique, only: :create

    def new
      @referrer = User.find_by(referral_code: params[:referral_code])
      @clique = (params[:clique].present? && params[:clique] == "enabled") ? params[:clique] : "disabled"
      @plan = params[:plan].present? ? params[:plan] : ""
      super
    end
    ...
    def after_sign_up_path_for(resource)
      ...
      if resource.agent?
        if (@clique == "enabled") && ["premium", "pro"].include?(@plan)
          return customer_checkout_index_path(clique: @clique, plan: @plan)
        else
          return select_subscription_path
        end
      end
    ...
    def configure_permitted_parameters
      devise_parameter_sanitizer.permit(:sign_up, keys: %i[first_name last_name phone_number role professional_role accept_referral_agreement confirmed_at clique])
    ...
    def set_clique
      @clique = params[:user].extract!(:clique)[:clique]
      @plan = params[:user].extract!(:plan)[:plan]
    end
  end
end
