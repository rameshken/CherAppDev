...
class ActivityMailer < ApplicationMailer
  ...
  # def home_buyers_newsletter_subscribe(email)
  ...
  #   # users.each do |e| 
  #   #   to_emails << e.email
  #   #   contact = User.find_by(email: e.email)
  #   #   @user = contact
  #   #   # return if check_bounces(e.email) || (contact && !contact.accept_notification?(type: :subscription, method: :email))
  #   #   mail from: SENDER_EMAIL, to: ['rameshkbdm@gmail.com'], subject: 'Get $15 for 15 minutes of your time - only 3 spots left!'
  #   #   # mail from: SENDER_EMAIL, to: e.email, subject: 'Get $15 for 15 minutes of your time - only 3 spots left!'
  #   # end
  #   contact = User.find_by(email: email)
  #   @user = contact
  #   return if check_bounces(email) || (contact && !contact.accept_notification?(type: :subscription, method: :email))
  #   mail from: SENDER_EMAIL, to: email, subject: 'Get $15 for 15 minutes of your time - only 3 spots left!', delivery_method_options: DRIP_CAMPAIGN_CONFIG
  ...
  def weekly_properties_drip(properties, user)
    ...
    contact = User.find_by(email: user.email)
    @user = contact

    return if check_bounces(user.email) || (contact && !contact.accept_notification?(type: :subscription, method: :email))
  ...
  def weekly_leads_drip(drip_params, agent)
    ...
    contact = User.find_by(email: agent.email)
    @user = contact

    return if check_bounces(agent.email) || (contact && !contact.accept_notification?(type: :subscription, method: :email))
