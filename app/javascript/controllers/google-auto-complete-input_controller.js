    ...
    google.maps.event.addListener(autocompleteInput, 'place_changed', () => {
      ...
      if (type === 'address' && place.formatted_address) {
        ...
        this.searchInputTarget.value = newPlace.replace(/, USA/, '');
          ...
          const newPlace = this.searchInputTarget.value;
        // this.searchInputTarget.value = newPlace.replace(/, .., USA/, '');
        this.extract_city_and_state(place.address_components)
      }
    });
  }

  extract_city_and_state(address_components){
    const stateComponent = address_components.find((component) => (
      component.types.includes('administrative_area_level_1')
    ));
    const state = stateComponent ? stateComponent.short_name : 'Not defined';

    const neighborhoodComponent = address_components.find((component) => (
      component.types.includes('neighborhood')
    ));

    const localityComponent = address_components.find((component) => (
      component.types.includes('locality')
    ));
    const city = neighborhoodComponent ? neighborhoodComponent.long_name : localityComponent.long_name;

    this.searchInputTarget.value = `${city}, ${state}`
  }
