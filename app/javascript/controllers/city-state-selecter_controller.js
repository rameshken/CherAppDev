...
export default class extends Controller {
  static targets = ['searchInput', 'geoLocationCity', 'submitButton', 'errorMessage']
    ...
    this.geocoder.geocode({ address: city }, (results, status) => {
      if (status === 'OK') {
        ...
        if (result.formatted_address && result.formatted_address.includes('USA')) {
          ...
          this.setInUsa();
          this.isInvalidLocation(result.formatted_address);
        }
        else {
          this.setOutUSA();
        }
  ...
  isInvalidLocation(formatted_address) {
    if (!formatted_address.includes(this.searchInputTarget.value)) {
      this.setinvalidLocation();
    }
  }
}
