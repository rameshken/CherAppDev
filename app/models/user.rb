...
class User < ApplicationRecord
  ...
  VALID_PHONE_NUMBER_REGEX = /\A(?:\+?\d{1,3}\s*-?)?\(?(?:\d{3})?\)?[- ]?\d{3}[- ]?\d{4}\z/.freeze
  ...
  validates :phone_number, phone: {allow_blank: true, types: [:voip, :mobile], countries: [:us, :in]}
  ...
  scope :concierge_contact, -> { find_by(email: 'ramisn26@live.in') }
