...
class ProfessionalContactCheckerService
  ...
  def find_new_professionals_batch(unacepted_channels)
    ...
    new_professionals_batch = User.kept
                                  .with_role(:agent)
                                  .with_clique
                                  .where.not(email: professionals)
                                  .limit(10)
    new_professionals_batch = User.kept.with_role(:agent).where.not(email: professionals).limit(10) if new_professionals_batch.blank?
