...
module OverviewHelper
  ...
  def chart_data(deeds)
    data = {}
    deeds = deeds.reverse
    deeds.each do |deed|
      data.store(deed&.dig('recording_date'), deed&.dig('sale_price'))
    end

    return data
  end

  def field_data_collection(data_collection)
    if data_collection.is_a?(Array)
    ...
    data_collection || 'Not Provided'
  end

  def format_date(data, format)
    unless data.nil?
      data = Date.strptime(data, "%Y-%m-%d").strftime(format)
    end
