...
class SimplyRets
  ...
  def self.get_cities
    cities = []
    AVAILABLE_VENDORS.each do |vendor|
      cities += cities_api_call(vendor)
    end

    return cities
  end

  def self.cities_api_call(vendor)
    response = RestClient::Request.execute(
      :method => :options, 
      :url => simply_rets_url('properties'), 
      :headers => {params: {vendor: vendor}}
    )

    return [] unless response.code == 200
    return JSON.parse(response)["fields"]["cities"]
  end
end
